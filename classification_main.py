...
def classification_main():
    ...
    train = pd.read_csv(TRAIN_FILE, delimiter='\1', header=0, names=['title', 'context', 'label'])
    test = pd.read_csv(TEST_FILE, delimiter='\1', header=0, names=['title', 'context', 'label'])
    valid = pd.read_csv(VALID_FILE, delimiter='\1', header=0, names=['title', 'context', 'label'])

    train_x, train_y = list(train['context']), list(train['label'])
    test_x, test_y = list(test['context']), list(test['label'])
    valid_x, valid_y = list(valid['context']), list(valid['label'])
